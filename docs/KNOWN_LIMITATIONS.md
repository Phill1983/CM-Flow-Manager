...
- **Phase 4C.1 extraction PoC** parses sanitized text-layer extracts. Image-only PDFs return `OCR_REQUIRED` (no OCR engine).
- **Phase 4C.2 PDF.js adapter** extracts a text layer locally; it is not OCR. PDF.js item order/spacing differs from `pdftotext -layout`. Shop-invoice `usł` lump labour keeps hours unresolved. Printed `Materiały dodatkowe` maps to `additionalCosts` (not labour). Desktop Repair Intelligence UI is not wired; soak is a developer CLI.
