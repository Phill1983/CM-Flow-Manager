...
| 2026-08-18 | Emergency Recovery **R&D direction**: Electron orchestrates; no JS password search; optional external local worker (GPU preferred); Hashcat-style / pdf2john-style are candidates only; spawn argv like qpdf; must not break other modules if unavailable. Implementation blocked on Engine Feasibility & Security Evaluation. | `docs/EMERGENCY_PDF_PASSWORD_RECOVERY.md` |
